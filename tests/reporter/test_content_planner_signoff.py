...
def _worksheet_with_data(rows: int = 5) -> openpyxl.worksheet.worksheet.Worksheet:
    """Return a worksheet with a header row plus `rows` data rows matching the Planner layout."""
    wb = openpyxl.Workbook()
    ws = wb.active
    from hype_frog.orchestration.content_planner import CONTENT_PLANNER_COLUMNS
    ws.append(list(CONTENT_PLANNER_COLUMNS))
    for i in range(rows):
        ws.append(["Home" if i == 0 else None, None, None, f"https://example.com/page-{i}/"]
                  + [None] * 15)
    return ws
...
def test_dropdown_added_to_columns_f_through_s() -> None:
...
def test_cf_formulas_reference_column_f(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert any("F2" in f for f in formulas)
...
def test_signoff_range_starts_at_column_6() -> None:
    """DataValidation sqref must include column F (index 6) for sign-off columns."""
    ws = _worksheet_with_data()
    apply_content_planner_signoff_rules(ws)
    dvs = list(ws.data_validations.dataValidation)
    # At least one DV must reference 'F2'
    sqrefs = [str(dv.sqref) for dv in dvs]
    assert any("F2" in s for s in sqrefs)
...
def test_column_widths_applied_for_planner_headers() -> None:
    ...
    assert ws.column_dimensions["F"].width == 20.0

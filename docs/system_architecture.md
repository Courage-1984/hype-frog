...
**Primary (visible, 20):** Table of Contents, Executive Briefing, Playbook, FixPlan, Quick Wins, Priority URLs, Content Optimisation Hub, Content & AI Readiness, Content Planner, Broken Link Impact, Robots.txt Analysis, SitemapQA, Main, AIOSEO Recommendations, Technical Diagnostics, Template & Duplication Risks, Snippet Opportunities, Link Intelligence, Script Inventory, Image Inventory.

**Advanced (hidden by default, linked from Executive Briefing/TOC, 7):** Issue Register (canonical backlog), CMS Action URLs, Redirects, Crawl Log, Competitor Benchmarks (when `--competitors` / `HF_COMPETITORS` set), DeltaFromPreviousRun, Audit Run Details.

Technical Diagnostics, Content & AI Readiness, Link Intelligence, Robots.txt Analysis, Snippet Opportunities, Script Inventory, and Image Inventory were promoted from the hidden Advanced group to the visible primary workflow. Content Hub Metrics and Anchor Text Audit were folded into Content & AI Readiness (their columns compute for every crawled URL, not just a curated subset). Link Inventory and Link Equity Map were folded into Link Intelligence: Link Inventory's deduplicated, SQLite-streamed rows became Link Intelligence's `Record Type = "Detail"` rows (appended after the `Record Type = "Summary"` rows, same sheet); Link Equity Map's columns were added to the Summary rows.

The legacy **Dashboard**, **Summary**, **IssueInventory**, **ResolvedIssues**, **Content Hub Metrics**, **Anchor Text Audit**, **Link Inventory**, and **Link Equity Map** sheets have all been retired — none are exported anymore (Summary folded into Issue Register; ResolvedIssues folded into DeltaFromPreviousRun; Content Hub Metrics/Anchor Text Audit folded into Content & AI Readiness; Link Inventory/Link Equity Map folded into Link Intelligence). Executive Briefing is the sole executive landing tab; Issue Register is the sole issue-backlog tab. Full per-tab detail: [`docs/workbook_tabs.md`](workbook_tabs.md).

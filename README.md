...
| `src/hype_frog/crawler/` | Crawl engine | Handles network execution, retries/backoff, fetch-mode selection, and typed crawl-result assembly. |
| `src/hype_frog/pipeline/` | Enrichment engine | Applies post-crawl transforms, scoring glue, sanitisation helpers, and graph analysis before reporting. |
| `src/hype_frog/reporter/` | Reporting engine | Builds Excel sheets with separate I/O, formatting, row-shaping, TOC/navigation, and guardrails modules. |
